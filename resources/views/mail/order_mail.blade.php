    ...
    <table>
        ...
        @foreach($order->products as $key =>$product)
            <tr>
                ...
                <td style='padding: 10px; border: #e9e9e9 1px solid;'>{{$product->price}} ₽</td>
                <td style='padding: 10px; border: #e9e9e9 1px solid;'><span class="badge">{{$product->pivot->quantity}}</span><br/>
                <td style='padding: 10px; border: #e9e9e9 1px solid;'>{{$product->getPriceForCount()}} ₽</td>
            </tr>
        @endforeach
        </tbody>
    </table>
    <p>Общая сумма: <b>{{$fullSum}} $</b></p>
